...
- different_damp.zip: Input files, results and analysis data for several examples with different values of the friction in the Langevin equation ($$\mathrm{damp}$$) from 0.01 to 1.5, in order to illustrate the relation between $$\mathrm{damp}$$ and $$D$$ (in all cases $$T^* =1$$ and $$m^* =1$$).
